...
function VerifyOTPContent() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (sessionResponse.ok && sessionData?.user) {
        if (sessionData.user.role === "external") {
          console.log("External User Company ID:", sessionData.user.companyId);
        }
        ...
        setTimeout(() => {
          if (!sessionData.user.onboarderStatus) {
            // router.push("/external/auth/onboarding");
        ...
        if (retryData.user.role === "external") {
          console.log("External User Company ID (Retry):", retryData.user.companyId);
        }
        ...
        setTimeout(() => {
          if (!retryData.user.onboarderStatus) {
            // router.push("/external/auth/onboarding");
            setShowOnboardingModal(true);
          } else if (retryData.user.role === "internal") {
            router.push("/internal/dashboard");
          } else if (retryData.user.role === "external") {
            router.push("/external/dashboard");
          } else {
            router.push("/external/dashboard");
          }
  ...
  const resendOTP = async () => {
    ...
    try {
      ...
      const response = await fetch("/api/AccountManager/WAVExternalUser/send-otp", {
        method: "POST",
        headers: {
        ...
        <DialogContent className="p-6 rounded-lg shadow-lg bg-white dark:bg-gray-900">
          <DialogHeader>
            ...
            <DialogDescription className="mt-2 text-gray-600 dark:text-gray-300">
              You are one step away! Tell us something about yourself before we take you to our
              amazing dashboards.
            </DialogDescription>
          </DialogHeader>
