...
interface AssociatedPractice extends AssociatedEntity {
  entitySubtype?: string;
  lifecycleStage?: string;
  location?: string;
}

export function PracticesTab({ practice }: PracticesTabProps) {
  const [searchTerm, setSearchTerm] = useState("");
  const [sortConfig, setSortConfig] = useState<{
    key: keyof AssociatedPractice;
    direction: "ascending" | "descending";
  }>({ key: "name", direction: "ascending" });
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;
  const [isUnlinkDialogOpen, setIsUnlinkDialogOpen] = useState(false);
  const [selectedPractice, setSelectedPractice] = useState<AssociatedPractice | null>(null);
  const [isUnlinking, setIsUnlinking] = useState(false);

  // Filter practices from associated entities
  const practices = practice.associatedEntities?.filter(
  ...
  const requestSort = (key: keyof AssociatedPractice) => {
  ...
  const handleUnlink = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/EntityUnlinking/PRACTICE/${practice.id}`,
        {
          method: "POST",
          headers: {
      ...
      // Remove the unlinked practice from the list
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-gray-200 dark:bg-gray-800">
                ...
                <TableHead className="text-black dark:text-white">Stage</TableHead> */}
            ...
            <TableBody>
              {paginatedPractices.map((practice, index) => (
                <TableRow key={practice.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  {/* <TableCell>{practice.entitySubtype || "N/A"}</TableCell>
                  <TableCell>
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-200">
                      {practice.lifecycleStage || "N/A"}
                    </span>
                  </TableCell> */}
                  <TableCell>
      ...
      <AlertDialog open={isUnlinkDialogOpen} onOpenChange={setIsUnlinkDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to unmap {selectedPractice?.name} from this practice? 
              This action will remove the association between the practices.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
