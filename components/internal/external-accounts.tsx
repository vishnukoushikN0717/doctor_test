...
import { useTheme } from "next-themes"; // Import useTheme hook (add this to your project if not already there)
...
export function ExternalAccounts() {
  ...
  const handleSendInvitation = async (email: string) => {
    setIsInviting(email);
    try {
      const response = await fetch('/api/AccountManager/WAVExternalUser/send-invitation', {
        method: 'POST',
        headers: {
          'accept': '*/*',
    ...
    try {
      ...
      return data.filter((user: User) => {
        ...
        const roleMatch = !userLevel ||
          user.userRole?.toLowerCase().replace(/[\s-]+/g, '') ===
          userLevel.toLowerCase().replace(/[\s-]+/g, '');
        ...
        <div className="flex items-center justify-end gap-1">
          ...
          <Button
            ...
            onClick={(e) => {
              ...
              router.push(`/internal/dashboard/edit-external-profile/${encodeURIComponent(item.id)}`);
            }}
          >
          ...
          <Button
            ...
            onClick={(e) => {
              ...
              // Set preference before navigating
              localStorage.setItem("accountTabPreference", "external");
              router.push(`/internal/dashboard/external-profile/${encodeURIComponent(item.id)}`);
            }}
          >
        ...
        <div className={`flex items-center justify-between gap-4 p-3 rounded-lg shadow-md border mb-4 ${theme === 'dark'
          ...
          <Button
            ...
            onClick={() => {
              // Set preference before navigating
              localStorage.setItem("accountTabPreference", "external");
              router.push("/internal/dashboard/createExternal");
            }}
          >
