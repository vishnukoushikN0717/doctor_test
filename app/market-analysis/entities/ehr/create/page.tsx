...
const formSchema = z.object({
  name: z.string().min(2, "EHR name must be at least 2 characters"),
  entitySubtype: z.string().min(1, "EHR type is required"),
...
export default function CreateEHR() {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      const apiPayload = {
        ...
        // noOfAssociatedCareProviders: values.noOfAssociatedCareProviders ? parseInt(values.noOfAssociatedCareProviders) : 0,
        // noOfPatients: values.noOfPatients || "",
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Failed to create EHR");
      }

      const createdEHR = await response.json();
      const newEhrId = createdEHR.result.id;
      setEhrId(newEhrId);
      setShowSuccessModal(true);
      toast.success("EHR created successfully");
    } catch (error) {
      const errorMessage = (error as Error).message || "Failed to create EHR";
      console.error("Error creating EHR:", errorMessage);
  ...
  const handleClosePopup = () => {
    ...
    router.push("/market-analysis/entities/ehr/landing");
  };
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="border-b pb-4">
        ...
        <h1 className="text-2xl font-semibold mt-2">Create EHR</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Fill in the details below to create a new EHR entity
        </p>
      </div>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="flex items-center gap-2">
                            ...
                            <RequiredLabel>EHR Name</RequiredLabel>
                          </FormLabel>
                          <FormControl>
                            <Input {...field} />
                          </FormControl>
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="flex items-center gap-2">
                            ...
                            <RequiredLabel>EHR Type</RequiredLabel>
                          </FormLabel>
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl>
                              <SelectTrigger>
                                <SelectValue placeholder="Select EHR type" />
                              </SelectTrigger>
                            </FormControl>
                            <SelectContent>
            ...
            <div className="flex justify-end gap-4 pt-4">
              <Button
                type="button"
                variant="outline"
                onClick={() => router.push("/market-analysis/entities/ehr/landing")}
                disabled={isSaving}
