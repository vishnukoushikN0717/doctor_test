...
export default function EditAncillary() {
  ...
  useEffect(() => {
    const fetchAncillary = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/EntityData/Entity/${params.id}?EntityType=ANCILLIARY`, {
          method: "GET",
          headers: {
            accept: "*/*",
          },
        ...
        form.reset({
          ...
          noOfPhysicians: data.noOfPhysicians?.toString() || "",
        ...
        toast.error("Failed to load ancillary data");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to update ancillary");
      }

      toast.success("Ancillary updated successfully");
      router.push(`/market-analysis/entities/ancillaries/profile/${params.id}`);
    } catch (error) {
      console.error("Error updating ancillary:", error);
      toast.error("Failed to update ancillary");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          variant="outline"
          onClick={() => router.push(`/market-analysis/entities/ancillaries/profile/${params.id}`)}
