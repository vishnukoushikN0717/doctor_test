...
export default function EditEHR() {
  ...
  useEffect(() => {
    const fetchEHR = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/EntityData/Entity/${params.id}?EntityType=EHR`, {
          method: "GET",
          headers: {
            accept: "*/*",
          },
        ...
        form.reset({
          ...
          noOfAssociatedCareProviders: data.noOfAssociatedCareProviders?.toString() || "",
          noOfPatients: data.noOfPatients?.toString() || "",
        });
      } catch (error) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      const apiPayload = {
        ...
        noOfPatients: values.noOfPatients ? parseInt(values.noOfPatients) : 0,
      ...
      if (!response.ok) {
        throw new Error("Failed to update EHR");
      }

      toast.success("EHR updated successfully");
      router.push(`/market-analysis/entities/ehr/profile/${params.id}`);
    } catch (error) {
      console.error("Error updating EHR:", error);
      toast.error("Failed to update EHR");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="flex h-[calc(100vh-4rem)] items-center justify-center">
        ...
        <span className="ml-2">Loading EHR details...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          variant="outline"
          onClick={() => router.push(`/market-analysis/entities/ehr/profile/${params.id}`)}
