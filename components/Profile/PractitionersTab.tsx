...
interface Practitioner extends AssociatedEntity {
  jobTitle?: string;
  email?: string;
  phoneNo?: string;
  location?: string;
...
export function PractitionersTab({ practice }: PractitionersTabProps) {
  ...
  const practitioners = practice.associatedEntities?.filter(
  ...
  const handleUnlink = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Failed to unmap practitioner");
      }

      toast.success(`Practitioner ${selectedPractitioner.name} has been successfully unmapped`);
      
      // Remove the unlinked practitioner from the list
      const updatedPractitioners = practitioners.filter(
  ...
  return (
    <div className="space-y-6">
      ...
      <AlertDialog open={isUnlinkDialogOpen} onOpenChange={setIsUnlinkDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Unmap Practitioner</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to unmap {selectedPractitioner?.name} from this practice? 
              This action will remove the association between the practitioner and the practice.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
