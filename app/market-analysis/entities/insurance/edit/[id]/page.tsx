...
export default function EditInsurance() {
  ...
  useEffect(() => {
    const fetchInsurance = async () => {
      try {
        setIsLoading(true);
        const response = await fetch(`/api/EntityData/Entity/${params.id}?EntityType=INSURANCE`, {
          method: "GET",
          headers: {
            accept: "*/*",
          },
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      router.push(`/market-analysis/entities/insurance/profile/${params.id}`);
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="flex h-[calc(100vh-4rem)] items-center justify-center">
        ...
        <span className="ml-2">Loading insurance details...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          variant="outline"
          onClick={() => router.push(`/market-analysis/entities/insurance/profile/${params.id}`)}
