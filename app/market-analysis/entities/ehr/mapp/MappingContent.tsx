    ...
    const fetchEntityData = async () => {
      try {
        ...
        const response = await fetch(
          `https://dawaventity-g5a6apetdkambpcu.eastus-01.azurewebsites.net/api/Entity/${entityId}?EntityType=EHR`,
          {
            method: "GET",
            headers: {
              accept: "*/*",
        ...
        setEntityData(data);
        setAssociatedEntities(data.e_AssociatedEntitys || []);
      } catch (error) {
  ...
  const handleAddEntities = async (type: string) => {
    ...
    try {
      ...
      const entityTypeFromData =
        entityData?.entityType?.toUpperCase() || "EHR";
      const response = await fetch(
        `/api/EntityLinking/${entityTypeFromData}/${entityId}`,
        {
          method: "POST",
          headers: {
