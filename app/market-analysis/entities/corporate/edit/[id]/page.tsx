...
export default function EditCorporate() {
    ...
    const fetchCorporate = async () => {
      try {
        ...
        const response = await fetch(
            // `https://dawaventity-g5a6apetdkambpcu.eastus-01.azurewebsites.net/api/Entity/${params.id}?EntityType=CORPORATE`, 
            `/api/EntityData/Entity/${params.id}?EntityType=CORPORATE`, 
            
            // /api/Entity/${params.id}?EntityType=CORPORATE
            {
              method: "GET",
              headers: {
                accept: "*/*",
        ...
        form.reset({
          corporateGroupName: data.name || "",
          corporateType: data.entitySubtype || "",
          ...
          noOfEmployees: data.noOfEmployees?.toString() || "",
          logo: data.logo || "",
          locationImage: data.locationImage || "",
          noOfLocations: data.noOfLocations?.toString() || "",
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      const response = await fetch(`/api/EntityData/Entity/${params.id}?EntityType=CORPORATE`, {
        method: "PUT",
        headers: {
      ...
      if (!response.ok) {
        throw new Error("Failed to update corporate");
      }

      toast.success("Corporate updated successfully");
      router.push(`/market-analysis/entities/corporate/profile/${params.id}`);
    } catch (error) {
  ...
  if (isLoading) {
    return (
      <div className="flex h-[calc(100vh-4rem)] items-center justify-center">
        ...
        <span className="ml-2">Loading corporate details...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 pb-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          variant="outline"
          onClick={() => router.push(`/market-analysis/entities/corporate/profile/${params.id}`)}
