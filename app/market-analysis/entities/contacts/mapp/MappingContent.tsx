...
export default function MappingPage() {
    ...
    const fetchEntityData = async () => {
      try {
        ...
        const response = await fetch(
          `https://dawavpersona-hvgye7gjbmf6h6fu.eastus-01.azurewebsites.net/api/Contact/${entityId}`,
          {
            method: "GET",
            headers: {
              accept: "*/*",
  ...
  const handleAddEntities = async (type: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/EntityLinking/CONTACT/${entityId}`,
        {
          method: "POST",
          headers: {
  ...
  const handleBack = () => {
    router.push(`/market-analysis/entities/contacts/landing`);
  };
